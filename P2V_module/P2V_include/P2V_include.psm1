...
$my_new_variable = @("dlg.WW.ADM-Services.P2V.access.production",
...
{ # function to sync user's base data AD <> P2V
  ...
  P2V_print_object($user_selected)
  write-output $linesep
 ...
 #              authenticationMethod = "SAML2"
		...
		$usr_asis      = P2V_get_userlist($t) |where-Object {($($_.logOnId) -eq $user_selected.logOnId -or $($_.displayName) -like "$($user_selected.SAMAccountName)")}
		
		if ($usr_asis.count -gt 1) 
		{
			 $usr_asis= $usr_asis |out-gridview -Title "select user to sync" -outputmode single 
		}
		
		write-output $linesep
		$form1 -f " TENANT: $tenant"
        P2V_print_object($usr_asis|select id, displayName,logonId,description,EmailAddress,isaccountlocked,isdeactivated)
	
     	$usr_asis |% {$u_list_P2V_idx[$_.logOnId]=$_ }
		
		write-progress "P2V users loaded"
			
		# $u_list_P2V_idx|convertto-json |out-host
		# $user_profile_list=get_PS_userlist $tenants[$ts] |  where-Object {($($_.logOnId) -eq $user_selected.logOnId) }
	
		
		
		$usr_tobe.ID = $usr_asis.ID
		write-output $linesep		
		write-output ($form4_1 -f "attribute","action", "Active Directory","$tenant")
		write-output $linesep	
		if ($($usr_tobe.logOnId) -eq $($usr_asis.logOnId)) 		 
		{  
		   $action= "[------]" 
        } 
		else 
		{  
		   ...
		   $change_ops += [PSCustomObject]@{  
                       op     = "replace"
                       path   = "/logOnId"
                       value  = $($user_selected.logOnId)			
				    }  
		}
		write-output ($form4_1 -f "logOnId",$action , "$($user_selected.logOnId)","$($usr_asis.logOnId)")
		
		if ($($user_selected.displayName) -eq $($usr_asis.displayName)) 
		{$action= "[------]"} 
		else 
		{
			$action= "[CHANGE]"
			 $change_ops += [PSCustomObject]@{  
                       op     = "replace"
                       path   = "/displayName"
                       value  = $($user_selected.displayName)			
				    }  
		}
		write-output ($form4_1 -f "displayName",$action, "$($user_selected.displayName)","$($usr_asis.displayName)")
		
		
		if ($($user_selected.department) -eq $($usr_asis.description))  
		{$action= "[------]"} 
		else 
		{
			$action= "[CHANGE]"
			 $change_ops += [PSCustomObject]@{  
                       op     = "replace"
                       path   = "/description"
                       value  = $($user_selected.department)			
				    }  
		}
		write-output ($form4_1 -f "description",$action , "$($user_selected.department)","$($usr_asis.description)")
		
		if ($($user_selected.emailAddress) -eq $($usr_asis.emailAddress)) 
		{$action= "[------]"} 
		else 
		{
			$action= "[CHANGE]"
			 $change_ops += [PSCustomObject]@{  
                       op     = "replace"
                       path   = "/emailAddress"
                       value  = $($user_selected.emailAddress)			
				    }  
		}
		write-output ($form4_1 -f "emailAddress",$action , "$($user_selected.emailAddress)","$($usr_asis.emailAddress)")
		<# write-output ($form3_2 -f "description", "$($user_selected.description)","$($u_list_P2V.description)")
		write-output ($form3_2 -f "description", "$($user_selected.description)","$($u_list_P2V.description)")
		write-output ($form3_2 -f "description", "$($user_selected.description)","$($u_list_P2V.description)") #>
		
				
		
		#$change_ops | out-gridview -Title "change-ops to apply" -wait
		
		if (-not $yes_all_tenants) { $yes_all_tenants= ask_continue  -title "Apply changes to all tenants?" -msg "apply changes for $($user_selected.displayName) to all tenants?" }
		
		
		if (-not (update_PS_user -tenant $t -user_profile_old $usr_asis -user_profile_new $usr_tobe -all_write $yes_all_tenants)) {write-output "ERROR"}
...
foreach ($ts in $tenants.keys)
{
	...
	$UsersFromTenantList= P2V_get_userlist -tenant $t 
	...
	#$UsersFromTenantList=$UsersFromTenantList|Where { (! $_.isDeactivated) }
	...
	write-output ($form_status -f " #users active and NEVER logged in: ",$UsersFromTenantListNever.count)
    ...
    # retrieve all users incl. workgroups
	 ...
	 $cont=ask_YesNoAll -title "[$tenant] deactivate users?" -msg "deactivate $($UsersFromTenantListNever.count) users in $tenant ?"
	...
	write-output $linesep
    write-output ($form_status -f " Active and not  logged in for $max_days_lock days:",$UsersFromTenantListMaxDays.count)
	...
	write-output ($form_status -f "users selected to lock",$UsersFromTenantListMaxDays.count)
     $cont=ask_YesNoAll -title "[$tenant] lock users?" -msg "lock $($UsersFromTenantListMaxDays.count) users in $tenant ?"
	 write-output ($form1 -f "[$cont] selected")
	 switch ($cont)
	 {
                 Yes    {$do_it =$true; Break}
	...
	
	 {
		 foreach ($u in $UsersFromTenantListMaxDays)
		 {
			...
			$updateUserOperations += [PSCustomObject]@{
                    op = "replace"
                    path = "/description"
			        value = "[locked due to inactivity $max_days_lock] - $($u.description)"
            }
			# 
...
#-----------------------------------------------------------------
Function show_progress ([int]$count=0,[bool]$type=$false)
{
   if ($type)	
   { 
    write-host -nonewline -ForegroundColor green "[ $count ]`r"
   }
   else
   {
    $progress=@("[/]`b`b`b","[-]`b`b`b","[\]`b`b`b","[|]`b`b`b")
      #$i=$i%4
    write-host -nonewline -ForegroundColor green $progress[$count%4]
   }
}
