...
foreach ($ts in $tenants.keys)
{
  ...
  write-host ($form1 -f ">> checking $tenant")
  ...
  foreach ($currentuser in $P2V_U.keys)
  {
	  ...
	  write-host -nonewline "$($t.tenant):[$bd]`r"
		...
		#	     $form_status -f "$currentuser == $g_a","[SKIP]"
			   ...
			   { #as-is: Allow- to-be Allow+ -> ADD
		         ...
		         write-host (  $form_status -f "$currentuser += $g_a","[ADD]")
			   ...
			   { #as-is: Deny+ to-be Deny- -> DEL
  		         write-host ( $form_status -f "$currentuser -= $g_d","[DEL]")
				 $change_ops  = [PSCustomObject]@{
                              op = "remove"
                              path = "/users/$uid"
                              value = ""
		...
		{ # -> User is NOT allowed -> add to .deny group , remove from .allow groupDENY
			  ...
			  { #as-is: Allow+ to-be Allow- -> DEL
  		        write-host ( $form_status -f "$currentuser -= $g_a","[DEL]")
				$change_ops  = [PSCustomObject]@{
                              op = "remove"
                            path = "/users/$uid"
                           value = ""
		...
		#	    $form_status -f "$currentuser == $g_d","[SKIP]"
			  ...
			  { #as-is: Deny- to-be Deny+ -> ADD
  		         write-host ( $form_status -f "$currentuser += $g_d","[ADD]")
				$change_ops  = [PSCustomObject]@{
                            op = "add"
                          path = "/users/$uid"
                         value = ""}
	...
	#    $form_status -f "$currentuser == $bd_base_group","[SKIP]"
	   ...
	   {
	#     $form_status -f "$currentuser -= $bd_base_group","[DEL]"
		 $change_ops  = [PSCustomObject]@{
                              op = "remove"
                            path = "/users/$uid"
                           value = ""
	 ...
	 { # user  has NO base  BD access yet
	    if ($bd_base) 
		{
	#	   $form_status -f "$currentuser += $bd_base_group","[ADD]"
		   $change_ops  = [PSCustomObject]@{
                            op = "add"
                          path = "/users/$uid"
                         value = ""}
   ...
   { 
     ...
     $a1="$k.allow  total:$($bd_group_members[$k.tostring()].allow.count)"
	 $a2="  ops: $($updateOperations[$ka].count)"
	 $d1="$k.deny   total: $($bd_group_members[$k.tostring()].deny.count )"
	 $d2="  ops: $($updateOperations[$kd].count)"
 	
		    $form2_2 -f "$a1 $a2", "$d1 $d2"
	}
		
	$form2 -f "total ADD ops: $acount","total DEL ops: $dcount"
	...
	foreach ($i in $updateOperations.keys)
    {
        if ($updateOperations[$i].Count -gt 0) 
		{
            ...
            if (! $do_all)
			{
			  ...
			  If ($cont -like "a") {$do_all=$true}
            }			  
			  
			if ($do_all -or ($cont -like "y"))
            {			
