...
{ # function to verify and select user  via GUI 
  ...
  #--------------------------------
   ...
   while (!$ad_user_selected)
	 {
	 	while (-not $searchstring) {$searchstring= Read-Host "|> Please enter user-searchstring (0=exit)"}
		...
		#$u_res=Get-ADUser -SearchBase "DC=ww,DC=omv,DC=com" -Filter { (Givenname -like $ad_user) -or (Surname -like $ad_user) -or (Name -like
		...
		else
		{
		  ...
		  #foreach ($element in $ad_user_selected.PSObject.Properties)  #
        #	{
	#		   $element.Value=($element.Value).trim()
			   #write-host $($element.Name)+"="+"[$($element.Value)]"
#		       $form2_2 -f "$($element.Name)","$($element.Value)" 
#            }
			#$linesep|out-host
		    #$ad_user_selected|format-list|out-host
		}
	}	 
...
{# function to add 1 user to P2V_tenant
  ...
  write-host ($form1 -f "add $($user_profile.logonID) to $($tenant.tenant)")
  ...
  write-host ($form_status -f $user_profile.displayName,$rc )
  out-host  
  return $r
}
#---------------------------------------------------
...
{
   if ($debug) { $form1 -f "loading profiles from $profile_file" }
...
Function check_datagroup_dependencies ( [System.Collections.ArrayList] $grouplist,[bool] $debug=$false)
{
	...
	write-Output ($form1 -f ">> check_datagroup_dependencies ")
	#write-Output $linesep
    # check datagroups 
...
Function check_BD_dependencies ( $login,[System.Collections.ArrayList] $grouplist,[bool] $debug=$false)
{
   ...
   # write-host $linesep
   write-Output ($form1 -f ">> check_BD_dependencies ")
   # write-host $linesep	
...
Function check_license_dependencies ( [System.Collections.ArrayList] $grouplist,[bool] $debug=$false)
{
   ...
   # write-host $linesep
   write-Output ($form1 -f ">> check_license_dependencies ")
   ...
   if ($l_h -and $l_l) 
   {
      write-Output ($form_status -f $light,"[DEL]")
      $grouplist.Remove("$light")| Out-Null
   }
   if (!$l_h -and !$l_l) 
   {
      write-Output ($form_status -f $light,"[ADD]")
...
{  #  check templates
   ...
   #write-host $linesep
   write-Output ($form1 -f ">> check_template_dependencies ")
   ...
   foreach( $tag in $tag_triple)
   {
	  ...
	  if ($state0 -eq 0)                         
	  {$state1 = $deny_code; $state2= "DENY"}  # default = DENY
	  
	  if ($grouplist -contains $($tag.deny))   
	  {$state0 = $state0 -bor $deny_code;$state1 = $deny_code; $state2= "DENY"}
	  
	  if ($grouplist -contains $($tag.readonly)) 
	  {$state0 = $state0 -bor $readonly_code;$state1 = $readonly_code; $state2= "R/O"}
	  
	  if ($grouplist -contains $($tag.allow))
	  {$state0 = $state0 -bor $allow_code;$state1 =  $allow_code; $state2= "FULL"}
	  ...
	  if ($state0 -ne $state1)
	  {
	     ...
	     if ($change0 -band $allowcode) 
	      { write-Output ($form_status -f $tag.allow,$activity )}
		
	  	 # more realistic ...
	     if ($change0 -band $readonly_code)
	      { write-Output ($form_status -f $tag.readonly,$activity )}
          
	     # very often ...
	     if ($change0 -band $deny_code)
	      { write-Output ($form_status -f $tag.deny,$activity )}
	  }
	    
    }
   
   #if ($debug)  {$linesep;$grouplist|format-wide -column 3;$linesep;pause}
   out-host
   if ($debug) {$grouplist|out-gridview -title "check_template_dependencies"}
   write-debug "grouplist:  $($grouplist.GetType().FullName)"
   return $grouplist
}
...
$global:form_user  ="|  {0,-10} {1,-55} {2,-55} |"
...
$global:form_user  ="|  {0,-5} {1,-34} {2,-34}  |"
