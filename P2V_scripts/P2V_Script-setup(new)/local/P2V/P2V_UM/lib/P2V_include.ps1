...
Function check_template_dependencies ( [System.Collections.ArrayList] $grouplist,[bool] $debug=$true)
{  #  check templates
   ...
   foreach( $tag in $tag_triple)
   {
	  ...
	  if ($state0 -ne $state1)
	  {
	     ...
	     if ($change0 -band $allowcode) 
	      { write-debug ($form_status -f $tag.allow,$activity )}
	     ...
	     if ($change0 -band $deny_code)
	      { write-debug ($form_status -f $tag.deny,$activity )}
	  }
...
$global:form5_2    ="|  {0,-40} {1,-40} {2,-40} {3,-40} {4,-8} |"
