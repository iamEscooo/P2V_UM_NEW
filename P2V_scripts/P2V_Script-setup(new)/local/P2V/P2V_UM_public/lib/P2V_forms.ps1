...
#  $usr_sel = @{}
  ...
  if ($xkey) {$u_input.Text = $xkey}else {$u_input.Text =  "<user - xkey>"}
  ...
  $u_search_button.Add_Click({
	 ...
	 if ($xkey) {$global:usr_sel = get_AD_user -xkey $xkey}
  	 else       {$global:usr_sel = get_AD_user -searchstring $u_input.Text}
	 if ($global:usr_sel)
	 {
	    $Userinfo.Lines =  " "
