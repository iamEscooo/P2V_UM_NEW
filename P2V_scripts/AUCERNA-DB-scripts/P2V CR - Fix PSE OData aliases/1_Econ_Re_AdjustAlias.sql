...
NULL
END AS New_Alias from [CashDFX].[Variable] 
)
Select *, 'UPDATE [CashDFX].[Variable] SET Alias = '''+New_Alias+''' WHERE VariableId = ' + cast(VariableId as NVARCHAR(10)) AS UPDATE_Statement from DataSet 
where New_Alias <> Alias
End 
Else
Begin
UPDATE [CashDFX].[Variable] 
SET 
