...
$profile_sel=$P2V_profile.getenumerator()|sort -property name|out-gridview -title "[$tenant]:>> select Profile(s) to apply"  -outputmode multiple
   ...
   foreach ($k in $user_ops.keys)
   {
	 ...
	 if ($user_ops[$k].count -gt 0 )
	 {
		$i_result= @{}
		 ...
		 foreach($res in $i_result.keys)
		 {
		  $form_status -f ">$res$($i_result[$res].response)",$res
		 }
