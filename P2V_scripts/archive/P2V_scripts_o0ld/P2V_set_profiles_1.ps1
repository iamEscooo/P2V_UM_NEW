...
$profile_sel=$P2V_profile|sort -property name|out-gridview -title "Select Profile to apply"  -outputmode multiple
...
foreach ($p in  $profile_sel) {
 $form1 -f $p.key
 $p.value|%{$form2 -f "","$_";  $($u_sel.id |select id,displayname )|write-log ("$($u_sel.id) / $($u_sel.displayname) will get profile : $($p.key)")}
