...
If(!(test-path $output_path)){New-Item -ItemType Directory -Force -Path $output_path }
 ...
 foreach ($i in $all_systems)
  {
	...
	if ($get_workgroups)
	{
	   ...
	   #"$tenantURL/PlanningSpace/api/v1/workgroups?include=users&include=allowedRoles" -Method GET -Headers @{'Authorization' = "Basic $base64AuthInfo"}
	
	   if (!$w_result) {$form2_1 -f "[ERROR]", "cannot contact $i !" ;break}
	   $w_result |sort-object -property id |%{$form2 -f $($_.id),$($_.name)}
	   $form1 -f "export to $u_file"
