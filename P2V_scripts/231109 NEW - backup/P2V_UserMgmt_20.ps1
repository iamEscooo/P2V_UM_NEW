...
$Button14.Add_Click({
   $TextBox1.lines = $Button14.text
	...
	(write-output ($global:usr_sel)+$linesep+(Get-Module -name "*P2V*")+$linesep+(Get-ADDomainController |select Hostname,Name,IPv4Address)+$linesep+(dir env:)+$linesep+ ($(import-csv $adgroupfile  )|convertto-json))	|out-string -Stream| foreach-object {
    $TextBox1.lines = $TextBox1.lines + $_
    $TextBox1.Select($TextBox1.Text.Length, 0)
    $TextBox1.ScrollToCaret()
    $ProgressBar1.Value = ($ProgressBar1.Value + 2 ) % 100
