...
param(
  [string]$system="ALL",
  [string]$conf_dir="\\somvat202005\PPS_share\P2V_Script-setup(new)\central\config\startup_message",
  [bool]$debug = $false
)#---
#
$show_info=$true
...
#
 $allowed_systems  = @("PROD","TEST","DEV","TRAINING","UPDATE","ALL","PETROVR","CASH","NAMR")
#

if ($allowed_systems -notcontains $system)
{
  write-host -ForegroundColor red "[$system] is not a valid argument"
  write-host "allowed values are:"
  $allowed_systems|% {write-host "  $_"}
  exit
}
...
{ # show header
 #>
   ...
   $conf_dir_msg="$conf_dir\startup_message"
   $msg_file="$conf_dir\message.csv"

   $InfoParams =@{}

   $msg_to_show=import-csv $msg_file  

   if ($debug)
    {
	  Write-host "system:    $system"
	  Write-host "msg_file:  $msg_file"
	  $msg_to_show|out-host
	}

	$msg_to_show=$msg_to_show|where { ($($_.app) -like "$system")-or ($($_.app) -like "ALL")}
   ...
   foreach ($m in $msg_to_show)
   {
     ...
     if (test-path ("$conf_dir\$($m.msgfile)"))
     {
        ...
        $body=Get-content -Path "$conf_dir\$($m.msgfile)"
#        "$conf_dir\$($m.msgfile)"
