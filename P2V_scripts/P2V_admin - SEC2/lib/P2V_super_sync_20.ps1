...
#-----------------------------------------------------------------      
	...
	$ADgroupLoadList =@("dlg.WW.ADM-Services.P2V.access.production","dlg.WW.ADM-Services.P2V.access.test","dlg.WW.ADM-Services.P2V.access.update","dlg.WW.ADM-Services.P2V.access.training","dlg.WW.ADM-Services.P2V.access.development")
...
#-----------------------------------------------------------------      
	...
	$searchdir="\\somvat202005\PPS_share\P2V_scripts\config"
	do
	{   # ask for filenames
		...
		#	$user_profile_list.userWorkgroups|fl|out-host
			...
			$user_profile_list.asis|% { 
				...
				{ 
				 ...
				 $compare_log += @(($form3_2 -f "[$_]","[$_]","[ - ]"))
			
				} 
				else
				{ 
					#Write-Output -NoEnumerate ($form3_2 -f "[$_]","-","[DEL]")
					$compare_log += @(($form3_2 -f "[$_]","-","[DEL]"))
