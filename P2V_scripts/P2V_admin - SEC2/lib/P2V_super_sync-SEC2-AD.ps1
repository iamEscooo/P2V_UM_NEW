...
#-----------------------------------------------------------------     
   ...
   { 
      $ADuser_profiles["$($u.logonID)"]= get_AD_user -searchstring $($u.logonID)
	  show_progress ($count_u++) 
   }
		...
		foreach ($user_selected in $userlist)
		{
			...
			{  # add user
			   ...
			   if ($debug){$to_be|format-list}
			    ...
			    if (($cont=ask_continue -title "Add user to tenant [$tenant]?" -msg "add user [$($user_selected.displayName)] / [$($user_selected.logOnId)] ?") -like "Yes")
			   {
