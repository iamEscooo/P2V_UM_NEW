...
# !!! IGNORE GID as it is different in each tenant !!!
...
foreach ($ts in $tenants.keys)
  {
    ...
    foreach ( $wg in $WorkgroupRolesFromCsv.keys)
	{
		...
		# change GID to original one from tenant as the one from csv might be wrong
		
		if ( $element.GID -ne $WorkgroupsFromTenant[$wg].id)
		{
			write-output ($form1 -f "changing GID from $($element.GID) to $($WorkgroupsFromTenant[$wg].id) for $($element.WGName)")|out-host
			$element.GID = $WorkgroupsFromTenant[$wg].id
		}
