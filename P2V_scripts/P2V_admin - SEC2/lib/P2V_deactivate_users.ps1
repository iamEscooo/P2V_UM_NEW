...
foreach ($ts in $tenants.keys)
{
	...
	$UsersFromTenantListNever=$UsersFromTenantList| where {( ! $($_.isDeactivated))}  # |out-gridview -Title "NEVER logged in"
    ...
    # retrieve all users incl. workgroups
	 ...
	 $UsersFromTenantListNever =  $UsersFromTenantListNever |out-gridview -Title "select user(s) to deactivate" -OutputMode multiple
     write-output ($form_status -f "users selected for de-activation: ",$($UsersFromTenantListNever.count))
	 $UsersFromTenantListNever|ft
