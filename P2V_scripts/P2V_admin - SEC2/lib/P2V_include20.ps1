...
### introduced Enabled				
		...
		If (!$ad_user_selected) {$form_err -f "ERROR","$searchstring not found or no user selected"|out-host;$ad_user_selected=""}
		else
		{
   		   if ($u_count -gt 1) 
		   {
...
Function get_userprofilesFromCsv
{ # get user profiles from csv file
   param(
   [string]$profile_file="",
   [bool]  $all=$False
   )
   ...
   if (Test-Path $profile_file) 
   {
	 ...
	 $all_u_p=import-csv -path $profile_file
...
{ #get-workgrouplist from Planningspace $tenant
  ...
  $tenantURL      ="$($tenant.ServerURL)/$($tenant.tenant)"
  $base64AuthInfo ="$($tenant.base64AuthInfo)"
  $API_URL        ="$tenantURL/PlanningSpace/api/v1/workgroups?include=users&include=allowedRoles&include=deniedRoles"
...
#---------------------------------------------------
Function get_PS_rolelist($tenant)
{ #get-workgrouplist from Planningspace $tenant
  
  $tenantURL      ="$($tenant.ServerURL)/$($tenant.tenant)"
  $base64AuthInfo ="$($tenant.base64AuthInfo)"
  $API_URL        ="$tenantURL/PlanningSpace/api/v1/roles?include=allowedWorkgroups"
  
  $r_groups = Invoke-RestMethod -Uri $API_URL -Method GET -Headers @{'Authorization' = "Basic $base64AuthInfo"}
  if (!$r_groups) {write-host -ForegroundColor Red ($form_err -f "[ERROR] cannot contact $($tenant.tenant) !")}
...
Function update_PS_workgroup ($tenant, $group_profile_old, $group_profile_new)
{ #  function to update existing user in P2V_tenant
  #  [tenant] $tenant7
  #  [PS_user]  $user_profile_old
  #  [PS_user]  $user_profile_new
  ...
  #check if arguments are ok
  if (!$($tenant.tenant)) { write-error "tenant arguement emtpy!";return $False}
  if (!$($group_profile_new)){ write-error "user_profile_old emtpy!";return $False}
  if (!$($group_profile_old)){ write-error "user_profile_new emtpy!";return $False}
  if ( $($group_profile_new.id) -ne $($user_profile_old.id)) {write-error "user_profile id do not match!";return $False}
...
Function get_profiles ([bool] $debug=$false, [String] $ProfileDef=$profile_file)
{
   write-debug ($form1 -f "loading profiles from $ProfileDef")
   $csv_profiles=import-csv -path $ProfileDef |sort profile
   $profiles = @{}
   ...
   #if ($debug) {
	   $profiles|format-table |out-host 
	   #}
     
   write-output -NoEnumerate ($form_status -f "load profile definitions $ProfileDef","[DONE]")
...
Function check_BD_dependencies ( $login,[System.Collections.ArrayList] $grouplist,[bool] $debug=$true)
{
   $bd_assignments= @{}
   $all_bd= @{}
   $all_bd_ids= @{}
...
#----------------    special overruling definition   ----------------
   $bd_base_group="BD.base"
   $bd_groups=$config_path + "\P2V_BD-SEC2.csv"
#--------------------------------------------------------------------
   ...
   if ($debug) {$grouplist|out-gridview -Title "check_BD_dependencies" -wait}
...
Function check_license_dependencies ( [System.Collections.ArrayList] $grouplist,[bool] $debug=$false)
{
   ...
   if (!$l_h -and !$l_l) 
   {
    ...
    out-host
    if ($debug) {$grouplist|out-gridview -title "check_license_dependencies" -wait}
    write-debug "grouplist:  $($grouplist.GetType().FullName)"
    return $grouplist
} 
...
{  #  check templates
   ...
   $tag_conf    = $config_path + "\TAG_config-SEC2.csv"   
...
{ # function to print P2V objects (e.g. user-profile)
	...
	#foreach ($element in $object)   #.Properties
...
$global:config_path = "\\somvat202005\PPS_share\P2V_scripts\config"
...
$global:startdate = get-date -format "yyyy-MM-dd"
...
$global:form_chlogs="|  {0,-8} {1,-28} {2,-29} {3,-29} {4,-29}|" 
$global:form_logs=  "|  {0,-8} {1,-40} {2,-40} {3,-40} {4,-40}|" 
#$global:form_wg_r  ="|  {0,-5}/{1,-30}: {2,-5}/{3,-50} {4,-5} {5,-5}" 
$global:form_wg_r  ="|  {0,5}/{1,-30} {2,-5} {3,-5} {4,-5} {5,5}/{6,-44} " 
$global:emptyline  =($form1 -f "")
