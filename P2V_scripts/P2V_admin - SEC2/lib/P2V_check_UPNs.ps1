...
$u_list_P2V|% {$a =@{};
			   $a=$_;
			   $xt=$a.displayName.split("(");
				if ($xt.count -gt 1) {$xt=$xt[1].split(")")} else{ $xt=""};
			   $xkey=$xt[0]
			   $a | Add-Member -NotePropertyName xkey -NotePropertyValue $xkey;
...
foreach ($xkey in $($xkey_list_P2V.keys))
{
	...
	$UPN_local=Get-ADuser -filter {Name -like $xkey}|select UserPrincipalName
	if ($UPN_local)
	{
		
	    $entry = [PSCustomObject]@{
		     xkey=$xkey
	         UPN=$UPN_local.UserPrincipalName		 
			 LogonID=$xkey_list_P2V[$xkey].LogonID
			 status="ACTIVE"
			 lockstatus="UNLOCKED"
			 action="CHECK"
...
{ # for all P2V_users
   if ($spec_accounts -contains $u1) 
   {    # skip spec. accounts (e.g. s.ro.checkmw@petrom.com,..)
	   write-debug  ($form3_2 -f "$u1", "* special account","[SKIP]")
   } 
   else
   { # standardaccounts
    $change_ops=@()
    ...
    if ($u1_list_AD.ContainsKey($u1))
    {
	  ...
	  {$ccount +=1
	  write-debug ($form3_2 -f "$($u_ad.UserPrincipalName)", "found","[UPDATE]")
	  } 
	} else
    { #-- user not entitled -> deactivate
	  ...
	  if (!($u_p2v.isDeactivated))
	  { 
	     $form3_2 -f "$($u_p2v.id)/$($u_p2v.logonID)","in P2V but not in AD","[DEACTIVATE]"
...
# $form3_2 -f "$scount accounts", "already deactivated","[SKIP]"
$linesep
foreach ($u2 in  $u1_list_AD.keys)
{ # for all AD user of related ADgroup (base access)
   ...
   else
   {
	  ...
	  $form3_2 -f "$($u_ad_sel.UserPrincipalName)","in AD but not in P2V","[CREATE]"
...
$add_ops=$add_ops|out-gridview  -Title "select users to add" -OutputMode multiple
if (($cont=read-host ($form1 -f "add $($add_ops.count)/$acount users? (y/n)")) -like "y")	
{
...
$updateOperations=$updateOperations|out-gridview  -Title "select users to update" -OutputMode multiple
if (($cont=read-host ($form1 -f "update $($updateOperations.count)/$ccount users? (y/n)")) -like "y")	
{
...
#$deleteOperations.GetEnumerator() | % { $form4 -f "deactivate","$($u2_list_P2V[$($_.key.tostring())].logonID)","$($_.key)","$($_.value)" }
$deleteOperations=$deleteOperations|out-gridview  -Title "select users to DELETE" -OutputMode multiple
($deleteOperations|ConvertTo-Json)
if (($cont=read-host ($form1 -f "deactivate $($deleteOperations.count)/$dcount users? (y/n)")) -like "y")	
{
