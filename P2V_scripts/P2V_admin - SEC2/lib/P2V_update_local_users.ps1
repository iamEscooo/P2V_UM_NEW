...
Function Process-Users($usersFromCsv, $currentUsers, $analyzeOnly, $tenant)
{
    ...
    foreach($userKey in $usersFromCsv.Keys.GetEnumerator())
    {
        ...
        { # user exists in P2V
            ...
            {
                Write-Output " [$($userKey)] DisplayName will be changed to [$($userInCsv.DisplayName)]"|out-host
            ...
            {
                Write-Output " [$($userKey)] Description will be changed to $($userInCsv.Description)"|out-host
                $updateUserOperations += [PSCustomObject]@{
                    op = "replace"
                    path = "/description"
            ...
            {
                Write-Output " [$($userKey)] isDeactivated will be changed to $($userInCsv.IsDeactivated)"|out-host
                $updateUserOperations += [PSCustomObject]@{
                    op = "replace"
                    path = "/isDeactivated"
            ...
            {
                Write-Output " [$($userKey)] isAccountLocked will be changed to $($userInCsv.IsAccountLocked)"|out-host
                $updateUserOperations += [PSCustomObject]@{
                    op = "replace"
            ...
            {
                Write-Output " [$($userKey)] emailAddress will be changed to $($userInCsv.EmailAddress)"|out-host
                $updateUserOperations += [PSCustomObject]@{
                    op = "replace"
                    path = "/emailAddress"
        ...
        {
            Write-Output " New user $($userKey) will be created."|out-host
...
"+- users to delete ? -------------------------------------------------------------------+"
	...
	if (($cont=read-host ($form1 -f "deactivate [$($usersLookup.Count)] users? (y/n)")) -like "y")	
	{
        foreach($nonExistingUserKey in $usersLookup.Keys.GetEnumerator())
        {
           Write-host " deactivating user $($nonExistingUserKey) - skipped"
	...
	foreach($nonExistingUserKey in $newUsers.Keys)
    {
		Write-Warning " User $($nonExistingUserKey) exists in the CSV file but not yet in PlanningSpace."
	}
	...
	"+- users to update -------------------------------------------------------------------+"
	#$updateOperations|fl|out-host
	# ConvertTo-Json @($updateOperations)|out-host
...
do{
write-host -nonewline "|> select Local User file:  "
$UserInput_File = Get-FileName ($workingDir)
write-host "[$UserInput_File]"
...
$UsersFromCsv = Get-UsersFromCsv -usersFile $UserInput_File
# $usersFromCsv = Get-UsersFromCsv -usersFile $usersFile
# $userWorkgroupsFromCsv = Get-UserWorkgroupsFromCsv -usersFile $userWorkgroupsFile
...
if (!(Validate-CsvFile -checkfile $UserInput_File)) { exit }
...
$currentPSUsers = get_PS_userlist -tenant $t
...
Process-Users -usersFromCsv $usersFromCsv -currentUsers $currentPSUsers -analyzeOnly $analyzeOnly -tenant $t
