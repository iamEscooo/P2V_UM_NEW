...
#----- check whether xkey is member of workgroups in P2V
    ...
    # retrieve all users incl. workgroups
      $p2v_users=Invoke-RestMethod -Uri "$tenantURL/PlanningSpace/api/v1/users" -Method GET -Headers @{'Authorization' = "Basic $base64AuthInfo"}
      if (!$p2v_users) {$form2_1 -f "[ERROR]", "cannot contact $i !" ;break}
      $resp= $p2v_users|out-gridview -Title "select user" -outputmode multiple
      ...
      foreach ($r in $resp)
      {
		 ...
		 while (!($act=Read-host ($form1 -f "activate/deactivate (a/d) lock/unlock (l/u) exit (x) ?")	)) 
		 {
		    switch ($act)
			{
