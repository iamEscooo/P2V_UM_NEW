...
$output_path_base = "\\somvat202005\PPS_share\P2V_UM_data\LSP\output"
...
#----- start main part
  ...
  write-output  ($form1 -f "cleaning up output [$output_path]...")
  write-output ($form1 -f "createdir_ifnotexists -check_path $output_path  -verbose $true")
 ...
 out-host
  $all_adgroups = @()
  $searchdir="\\somvat202005\PPS_share\P2V_UM_data\LSP"
	do
	{   # ask for filenames
		Write-Output "|> select AD-Group file:  "
		$adgroupfile = Get-FileName -initialDirectory $searchdir
		Write-Output ($form1 -f "[$adgroupfile]")
        If(test-path $adgroupfile)
		{
              import-csv $adgroupfile |select displayName|% { $all_adgroups += @($($_.displayName))}
			  If (!$all_adgroups) { write-warning "error when loading AD-groups from $adgroupfile - retry"}
		} else
		{
			write-output " manual input:"
			while ($inp=read-host(" enter AD-Groups ( 0 = continue) : ") -ne "0")
			{
				 $all_adgroups += @($inp)
			}
		}
		
		write-output " all_adgroups to load:"
		$all_adgroups |out-host
  ...
  'ADgroup,ADgroup_short,LogonId,DisplayName,Domain,Description,IsDeactivated,IsAccountLocked,EmailAddress,UPN'
  foreach ($i in $all_adgroups)
  {
    # if ($check_group = Get-ADGroup -LDAPFilter "(SAMAccountName=$($i.ADgroup))")
    if ($check_group = Get-ADGroup -identity $i )
    {
   
      # headerline
      #     LogonId,Domain,DisplayName,Description,IsDeactivated,IsAccountLocked,EmailAddress
    
      Get-ADGroupMember -Identity $i|% {
	      $group_members["$($i)"]+=@($_);
	      if ($all_users.keys -notcontains $_ )
		  { 
		     $all_users["$($_)"] = Get-ADUser -identity $_ -properties Surname,GivenName,Name,UserPrincipalName, Department, EmailAddress 
		  }
	  }
	  $count=	$group_members["$($i)"].count
	
      #write-output ($form2 -f  "[$count]","users in $($i.ADgroup)" )
	  write-output ($form_status -f "$($i)",("[{0,3}]" -f $count))
    } else 
    { 
	  write-output ($form_status -f "$($i)",("[{0,3}]" -f "n/a"))
  ...
  $group_members| out-gridview -wait
  ...
  foreach ($g in $group_members.keys ) 
  {
    ...
    foreach ($u in $group_members["$g"])
    {
       ...
       Add-Content -Path $u_file     -Value ("$($e.Name),$OMV_domain,$($e.Surname) $($e.GivenName),$dep,FALSE,FALSE,") -PassThru
       Add-Content -Path $ad_file    -Value ("$g,$g_short,$($e.Name),$($e.Surname) $($e.GivenName),$OMV_domain,$dep,FALSE,FALSE,$($e.EmailAddress),$($e.UserPrincipalName)") -PassThru
      "$g,$g_short,$($e.Name),$($e.Surname) $($e.GivenName),$OMV_domain,$dep,FALSE,FALSE,$($e.EmailAddress),$($e.UserPrincipalName)"|out-host
       #$count++
    }
  }	
